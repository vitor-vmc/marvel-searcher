...
export class SearchRepository {
  ...
  getComics(data: SearchDataRequest) {
    let requestUrl = this._apiRequestModel.getUrl(data);

    this._httpClient
      .get<SearchReponse<Comic>>(requestUrl)
      .subscribe((response) => {
        console.log('retorno api (getComics) : ', response);
      });
  }

  getCreators(data: SearchDataRequest) {
    let request = this._apiRequestModel.getUrl(data);

    this._httpClient
      .get<SearchReponse<Creator>>(request)
      .subscribe((response) => {
        console.log('retorno api (getCreators) : ', response);
      });
  }

  getEvents(data: SearchDataRequest) {
    let requestUrl = this._apiRequestModel.getUrl(data);

    this._httpClient
      .get<SearchReponse<Events>>(requestUrl)
      .subscribe((response) => {
        console.log('retorno api (getEvents) : ', response);
      });
  }

  getSeries(data: SearchDataRequest) {
    let requestUrl = this._apiRequestModel.getUrl(data);

    this._httpClient
      .get<SearchReponse<Serie>>(requestUrl)
      .subscribe((response) => {
        console.log('retorno api (getSeries) : ', response);
      });
  }

  getStories(data: SearchDataRequest) {
    let requestUrl = this._apiRequestModel.getUrl(data);

    this._httpClient
      .get<SearchReponse<Story>>(requestUrl)
      .subscribe((response) => {
        console.log('retorno api (getStories) : ', response);
      });
  }
}
